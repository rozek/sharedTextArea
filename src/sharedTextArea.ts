...
/**** internalize ****/
  ...
  function internalize (Event:any):void {
    ...
    if (typeof Event.doc.Text === 'string') {
      Event.patches.forEach((Patch) => {            // update SelectionStart/End
        let Start:number, Length:number
        switch (Patch.action) {
          case 'del':
            Start  = Patch.path.pop() as number
            Length = (Patch.length || 1) as number
          ...
          case 'splice':
            Start  = Patch.path.pop() as number
            Length = (Patch.value as string).length

            if (Start < SelectionStart) { SelectionStart += Length }
            if (Start < SelectionEnd)   { SelectionEnd   += Length }
        }
      })
